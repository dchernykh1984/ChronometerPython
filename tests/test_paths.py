...
def test_app_path_joins_under_base_dir(monkeypatch) -> None:
    ...
    assert paths.app_path("data", "groupsList.txt") == Path(
        "/opt/app/data/groupsList.txt"
    )
...
def test_base_dir_frozen_uses_executable_folder(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        paths.sys, "executable", "/downloads/Chronometer", raising=False
    )
...
def test_base_dir_frozen_macos_app_uses_bundle_parent(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        paths.sys,
        "executable",
        "/Applications/Chronometer.app/Contents/MacOS/Chronometer",
        raising=False,
    )
