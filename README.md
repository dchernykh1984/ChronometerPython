...
[latest release](https://github.com/dchernykh1984/ChronometerPython/releases/latest):

| Platform | File |
| --- | --- |
| Windows (Intel/AMD) | `Chronometer-windows-x64.exe` |
| Windows (ARM) | `Chronometer-windows-arm64.exe` |
| macOS (Apple Silicon) | `Chronometer-macos-arm64.zip` |
| Linux (Intel/AMD) | `Chronometer-linux-x86_64` |
| Linux (ARM64) | `Chronometer-linux-aarch64` |
...
```bash
xattr -dr com.apple.quarantine "/path/to/Chronometer.app"
```
...
```bash
chmod +x Chronometer-linux-x86_64
./Chronometer-linux-x86_64
```
...
The program reads and writes its files (`data/groupsList.txt` and `data/http_config.txt`) **next to itself**, so give
